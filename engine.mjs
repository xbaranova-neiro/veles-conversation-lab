...
export function turn(old, raw, semantic=null) {
  ...
  if(intent==='stop'){
    ...
    switch(intent){
      ...
      case'mortgage':{const projects=/(?:где|посмотр|покаж).{0,30}проект/.test(t),family=/семейн/.test(t);if(projects||family)locked=true;reply=(projects?'Проекты могу показать здесь в чате. ':'')+(family?'Да, с семейной ипотекой работаем. Подходит ли программа именно вам, окончательно определяет банк. ':'Да, с ипотекой помогаем. ')+next(s);source=projects?['mortgage','portfolio']:['mortgage'];break;}
